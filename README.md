...
For the Query porition of CQRS you can then use whatever preferred interface language you want. Below we have used PHP to accomplish this action. It has allowed us to use a modern PHP framework called [Zend Framework 3](https://framework.zend.com/), modern styling via [Bootstrap](https://getbootstrap.com/), and the flexibility that if we want to change our interface to a different language (though we love PHP!). 
